			...
			for {
				...
				case *pb.TaskStreamResponse_NewPodLogsStream:
					...
					if podLogsTask.Payload == "cancel" {
						...
						_ctx, cancel := context.WithCancel(stream.Context())
						cancelMapMutex.Lock()
						cancelMap[podLogsTask.Id] = cancel
						cancelMapMutex.Unlock()
