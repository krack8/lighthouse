...
func (p *GetServiceAccountListInputParams) Process(c context.Context) error {
	...
	serviceAccountClient := cfg.GetKubeClientSet().CoreV1().ServiceAccounts(p.NamespaceName)
	limit := cfg.PageLimit
	if p.Limit != "" {
...
func (p *GetServiceAccountDetailsInputParams) Process(c context.Context) error {
	...
	serviceAccountsClient := cfg.GetKubeClientSet().CoreV1().ServiceAccounts(p.NamespaceName)
...
func (p *DeployServiceAccountInputParams) Process(c context.Context) error {
	ServiceAccountClient := cfg.GetKubeClientSet().CoreV1().ServiceAccounts(p.ServiceAccount.Namespace)
