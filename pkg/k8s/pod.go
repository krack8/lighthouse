...
func (p *GetPodDetailsInputParams) Process(c context.Context) error {
	...
	podsClient := cfg.GetKubeClientSet().CoreV1().Pods(p.NamespaceName)
	...
	podMetrics, err := cfg.GetMetricsClientSet().MetricsV1beta1().PodMetricses(p.NamespaceName).Get(context.TODO(), p.PodName, metav1.GetOptions{})
	if err != nil {
...
func (p *DeployPodInputParams) Process(c context.Context) error {
	podClient := cfg.GetKubeClientSet().CoreV1().Pods(p.Pod.Namespace)
