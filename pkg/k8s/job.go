...
func (p *GetJobListInputParams) Process() error {
	...
	jobClient := cfg.GetKubeClientSet().BatchV1().Jobs(p.NamespaceName)
	limit := cfg.PageLimit
	if p.Limit != "" {
...
func (p *GetJobInputParams) Process() error {
	...
	jobs := cfg.GetKubeClientSet().BatchV1().Jobs(p.NamespaceName)
...
func (p *DeployJobInputParams) Process(c context.Context) error {
	jobClient := cfg.GetKubeClientSet().BatchV1().Jobs(p.Job.Namespace)
