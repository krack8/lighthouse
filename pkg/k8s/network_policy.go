...
func (p *GetNetworkPolicyListInputParams) Process(c context.Context) error {
	...
	networkPolicyClient := cfg.GetKubeClientSet().NetworkingV1().NetworkPolicies(p.NamespaceName)
	limit := cfg.PageLimit
	if p.Limit != "" {
...
func (p *GetNetworkPolicyDetailsInputParams) Process(c context.Context) error {
	...
	networkPolicysClient := cfg.GetKubeClientSet().NetworkingV1().NetworkPolicies(p.NamespaceName)
