...
func (p *GetIstioGatewayListInputParams) Process(c context.Context) error {
	...
	istioGatewayClient := cfg.GetNetworkingV1Beta1ClientSet().Gateways(p.NamespaceName)
	limit := cfg.PageLimit
	if p.Limit != "" {
...
func (p *GetIstioGatewayDetailsInputParams) Process(c context.Context) error {
	...
	istioGatewaysClient := cfg.GetNetworkingV1Beta1ClientSet().Gateways(p.NamespaceName)
...
func (p *DeployIstioGatewayInputParams) Process(c context.Context) error {
	IstioGatewayClient := cfg.GetNetworkingV1Beta1ClientSet().Gateways(p.IstioGateway.Namespace)
