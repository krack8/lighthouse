...
func (p *GetEndpointSliceListInputParams) Process(c context.Context) error {
	...
	endpointSliceClient := cfg.GetKubeClientSet().DiscoveryV1().EndpointSlices(p.Namespace)
	limit := cfg.PageLimit
	if p.Limit != "" {
...
func (p *GetEndpointSliceDetailsInputParams) Process(c context.Context) error {
	...
	endpointSlicesClient := cfg.GetKubeClientSet().DiscoveryV1().EndpointSlices(p.Namespace)
