...
func (p *GetRoleBindingListInputParams) Process(c context.Context) error {
	...
	roleBindingClient := cfg.GetKubeClientSet().RbacV1().RoleBindings(p.NamespaceName)
	limit := cfg.PageLimit
	if p.Limit != "" {
...
func (p *GetRoleBindingDetailsInputParams) Process(c context.Context) error {
	...
	roleBindingsClient := cfg.GetKubeClientSet().RbacV1().RoleBindings(p.NamespaceName)
...
func (p *DeployRoleBindingInputParams) Process(c context.Context) error {
	RoleBindingClient := cfg.GetKubeClientSet().RbacV1().RoleBindings(p.RoleBinding.Namespace)
