...
func (p *GetReplicationControllerListInputParams) Process(c context.Context) error {
	...
	replicationControllerClient := cfg.GetKubeClientSet().CoreV1().ReplicationControllers(p.NamespaceName)
	limit := cfg.PageLimit
	if p.Limit != "" {
...
func (p *DeployReplicationControllerInputParams) Process(c context.Context) error {
	replicationControllerClient := cfg.GetKubeClientSet().CoreV1().ReplicationControllers(p.ReplicationController.Namespace)
