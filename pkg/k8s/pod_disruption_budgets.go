...
func (p *GetPodDisruptionBudgetsListInputParams) Process(c context.Context) error {
	...
	podDisruptionBudgetsClient := cfg.GetKubeClientSet().PolicyV1().PodDisruptionBudgets(p.NamespaceName)
	limit := cfg.PageLimit
	if p.Limit != "" {
...
func (p *GetPodDisruptionBudgetsDetailsInputParams) Process(c context.Context) error {
	...
	podDisruptionBudgetssClient := cfg.GetKubeClientSet().PolicyV1().PodDisruptionBudgets(p.NamespaceName)
...
func (p *DeployPodDisruptionBudgetsInputParams) Process(c context.Context) error {
	PodDisruptionBudgetsClient := cfg.GetKubeClientSet().PolicyV1().PodDisruptionBudgets(p.PodDisruptionBudgets.Namespace)
