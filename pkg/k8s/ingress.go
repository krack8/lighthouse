...
func (p *GetIngressListInputParams) Process(c context.Context) error {
	...
	ingressClient := cfg.GetKubeClientSet().NetworkingV1().Ingresses(p.NamespaceName)
	limit := cfg.PageLimit
	if p.Limit != "" {
...
func (p *GetIngressDetailsInputParams) Process(c context.Context) error {
	...
	ingresssClient := cfg.GetKubeClientSet().NetworkingV1().Ingresses(p.NamespaceName)
...
func (p *DeployIngressInputParams) Process(c context.Context) error {
	IngressClient := cfg.GetKubeClientSet().NetworkingV1().Ingresses(p.Ingress.Namespace)
