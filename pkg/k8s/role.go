...
func (p *GetRoleListInputParams) Process(c context.Context) error {
	...
	roleClient := cfg.GetKubeClientSet().RbacV1().Roles(p.NamespaceName)
	limit := cfg.PageLimit
	if p.Limit != "" {
...
func (p *GetRoleDetailsInputParams) Process(c context.Context) error {
	...
	rolesClient := cfg.GetKubeClientSet().RbacV1().Roles(p.NamespaceName)
...
func (p *DeployRoleInputParams) Process(c context.Context) error {
	RoleClient := cfg.GetKubeClientSet().RbacV1().Roles(p.Role.Namespace)
