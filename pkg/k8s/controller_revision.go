...
func (p *GetControllerRevisionListInputParams) Process(c context.Context) error {
	...
	controllerRevisionClient := cfg.GetKubeClientSet().AppsV1().ControllerRevisions(p.NamespaceName)
	limit := cfg.PageLimit
	if p.Limit != "" {
...
func (p *DeployControllerRevisionInputParams) Process(c context.Context) error {
	controllerRevisionClient := cfg.GetKubeClientSet().AppsV1().ControllerRevisions(p.ControllerRevision.Namespace)
