...
func (p *GetPvcListInputParams) Process(c context.Context) error {
	...
	pvcClient := cfg.GetKubeClientSet().CoreV1().PersistentVolumeClaims(p.NamespaceName)
	limit := cfg.PageLimit
	if p.Limit != "" {
...
func (p *GetPvcDetailsInputParams) Process(c context.Context) error {
	...
	pvcsClient := cfg.GetKubeClientSet().CoreV1().PersistentVolumeClaims(p.NamespaceName)
...
func (p *DeployPvcInputParams) Process(c context.Context) error {
	pvcClient := cfg.GetKubeClientSet().CoreV1().PersistentVolumeClaims(p.Pvc.Namespace)
