...
func (p *GetDaemonSetListInputParams) Process(c context.Context) error {
	...
	daemonSetClient := cfg.GetKubeClientSet().AppsV1().DaemonSets(p.NamespaceName)
	limit := cfg.PageLimit
	if p.Limit != "" {
...
func (p *GetDaemonSetDetailsInputParams) Process(c context.Context) error {
	...
	daemonSetsClient := cfg.GetKubeClientSet().AppsV1().DaemonSets(p.NamespaceName)
...
func (p *DeployDaemonSetInputParams) Process(c context.Context) error {
	daemonSetClient := cfg.GetKubeClientSet().AppsV1().DaemonSets(p.DaemonSet.Namespace)
