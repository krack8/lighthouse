...
@Component({
  selector: 'kc-k8s-stateful-sets',
  templateUrl: './k8s-stateful-sets.component.html',
  styleUrls: ['./k8s-stateful-sets.component.scss'],
  animations: [fadeInRight400ms]
})
export class K8sStatefulSetsComponent implements OnInit {
  icCircle = icCircle;
  ...
  getInstanceData(queryParam?: any): void {
    this.getStatefulSetStats(queryParam);
    this.loadingSpanner = true;
    this.namespaceService.getStatefulSets(queryParam).subscribe({
      next: res => {
        if (res?.status === 'error') {
  ...
  getStatefulSetStats(queryParam?: any): void {
    this.statsLoaded = true;
    this.namespaceService.getStatefulSetStats(queryParam).subscribe({
      next: res => {
        if (res?.status === 'error') {
          this.toastr.error(res?.message);
        }
  ...
  onDelete(item: any): void {
    ...
    dialogRef.afterClosed().subscribe((bool: boolean) => {
      if (bool === true) {
        this.namespaceService.deleteNamespaceStatefulset(item?.metadata?.name).subscribe(
          res => {
