...
@Component({
  selector: 'kc-k8s-role-binding-details',
...
export class K8sRoleBindingDetailsComponent implements OnInit {
  ...
  getInstanceData(): void {
    this.isLoading = true;
    this._namespaceService.getRoleBindingsDetails(this.namespaceInstance).subscribe({
      next: res => {
  ...
  onDelete(item: any): void {
    ...
    dialogRef.afterClosed().subscribe((bool: boolean) => {
      if (bool === true) {
        this._namespaceService.deleteNamespaceRoleBinding(item?.metadata?.name).subscribe(
          res => {
