...
@Component({
  selector: 'kc-k8s-resource-quota',
  templateUrl: './k8s-resource-quota.component.html',
  styleUrls: ['./k8s-resource-quota.component.scss']
})
export class K8sResourceQuotaComponent implements OnInit {
  icSearch = icSearch;
  ...
  getInstanceData(queryParam?: any): void {
    ...
    this.namespaceService.getResourceQuota(queryParam).subscribe({
      next: res => {
        if (res?.status === 'error') {
  ...
  onDelete(item: any): void {
    ...
    dialogRef.afterClosed().subscribe((bool: boolean) => {
      if (bool === true) {
        this.namespaceService.deleteNamespaceResourceQuota(item?.metadata?.name).subscribe(
          res => {
