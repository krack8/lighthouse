...
@Component({
  selector: 'kc-k8s-pod-disruption-budgets-details',
  templateUrl: './k8s-pod-disruption-budgets-details.component.html',
  styleUrls: ['./k8s-pod-disruption-budgets-details.component.scss']
})
export class K8sPodDisruptionBudgetsDetailsComponent implements OnInit {
  ...
  getInstanceData(): void {
    this.isLoading = true;
    this._namespaceService.getPdbDetails(this.namespaceInstance).subscribe({
      next: res => {
  ...
  onDelete(item: any): void {
    ...
    dialogRef.afterClosed().subscribe((bool: boolean) => {
      if (bool === true) {
        this._namespaceService.deleteNamespacePdb(item?.metadata?.name).subscribe(
          res => {
