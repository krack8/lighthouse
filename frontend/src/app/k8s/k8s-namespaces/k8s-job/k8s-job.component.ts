...
@Component({
  selector: 'kc-k8s-job',
  templateUrl: './k8s-job.component.html',
  styleUrls: ['./k8s-job.component.scss']
})
export class K8sJobComponent implements OnInit {
  icSearch = icSearch;
  ...
  getInstanceData(queryParam?: any): void {
    ...
    this.namespaceService.getJob(queryParam).subscribe({
      next: res => {
        if (res?.status === 'error') {
  ...
  onDelete(item: any): void {
    ...
    dialogRef.afterClosed().subscribe((bool: boolean) => {
      if (bool === true) {
        this.namespaceService.deleteJob(item?.metadata?.name).subscribe(
          res => {
