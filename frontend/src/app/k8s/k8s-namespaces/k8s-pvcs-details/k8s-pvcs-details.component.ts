...
@Component({
  selector: 'kc-k8s-pvcs-details',
  templateUrl: './k8s-pvcs-details.component.html',
  styleUrls: ['./k8s-pvcs-details.component.scss']
})
export class K8sPvcsDetailsComponent implements OnInit {
  ...
  icRight = icRight;
  title = 'Persistant Volume Claims';
  ...
  getInstanceData(): void {
    this.isLoading = true;
    this._namespaceService.getPvcDetails(this.namespaceInstance).subscribe({
      next: res => {
  ...
  onDelete(item: any): void {
    ...
    dialogRef.afterClosed().subscribe((bool: boolean) => {
      if (bool === true) {
        this._namespaceService.deleteNamespacePvc(item?.metadata?.name).subscribe(
          res => {
