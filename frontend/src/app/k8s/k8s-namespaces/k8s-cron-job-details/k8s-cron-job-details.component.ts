...
@Component({
  selector: 'kc-k8s-cron-job-details',
...
export class K8sCronJobDetailsComponent implements OnInit {
  ...
  getInstanceData(): void {
    this.isLoading = true;
    this._namespaceService.getCronJobDetails(this.namespaceInstance).subscribe({
      next: res => {
  ...
  onDelete(item: any): void {
    ...
    dialogRef.afterClosed().subscribe((bool: boolean) => {
      if (bool === true) {
        this._namespaceService.deleteCronJob(item?.metadata?.name).subscribe(
          res => {
