...
@Component({
  selector: 'kc-k8s-controller-revision-details',
  templateUrl: './k8s-controller-revision-details.component.html',
  styleUrls: ['./k8s-controller-revision-details.component.scss']
})
export class K8sControllerRevisionDetailsComponent implements OnInit {
  ...
  getInstanceData(): void {
    this.isLoading = true;
    this._namespaceService.getControllerRevisionDetails(this.namespaceInstance).subscribe({
      next: res => {
  ...
  onDelete(item: any): void {
    ...
    dialogRef.afterClosed().subscribe((bool: boolean) => {
      if (bool === true) {
        this._namespaceService.deleteNamespaceControllerRevision(item?.metadata?.name).subscribe(
          res => {
