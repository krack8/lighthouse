...
@Component({
  selector: 'kc-k8s-pod-disruption-budgets',
  templateUrl: './k8s-pod-disruption-budgets.component.html',
  styleUrls: ['./k8s-pod-disruption-budgets.component.scss']
})
export class K8sPodDisruptionBudgetsComponent implements OnInit {
  icSearch = icSearch;
  icInfo = icInfo;
  icDelete = icDelete;
  icEdit = icEdit;
  ...
  icRefresh = icRefresh;
  searchBy = 'name';
  title: any = 'Pod Disruption Budgets';
  ...
  getInstanceData(queryParam?: any): void {
    ...
    this.namespaceService.getPdb(queryParam).subscribe({
      next: res => {
        if (res?.status === 'error') {
  ...
  onDelete(item: any): void {
    ...
    dialogRef.afterClosed().subscribe((bool: boolean) => {
      if (bool === true) {
        this.namespaceService.deleteNamespacePdb(item?.metadata?.name).subscribe(
          res => {
