...
@Component({
  selector: 'kc-k8s-controller-revision',
  templateUrl: './k8s-controller-revision.component.html',
  styleUrls: ['./k8s-controller-revision.component.scss']
})
export class K8sControllerRevisionComponent implements OnInit {
  icSearch = icSearch;
  icInfo = icInfo;
  icDelete = icDelete;
  icEdit = icEdit;
  ...
  getInstanceData(queryParam?: any): void {
    ...
    this.namespaceService.getControllerRevision(queryParam).subscribe({
      next: res => {
        if (res?.status === 'error') {
  ...
  onDelete(item: any): void {
    ...
    dialogRef.afterClosed().subscribe((bool: boolean) => {
      if (bool === true) {
        this.namespaceService.deleteNamespaceControllerRevision(item?.metadata?.name).subscribe(
          res => {
  ...
  loadMore() {
    ...
    this.namespaceService
      .getControllerRevision(queryParam)
