...
@Component({
  selector: 'kc-k8s-certificates-details',
  templateUrl: './k8s-certificates-details.component.html',
  styleUrls: ['./k8s-certificates-details.component.scss']
})
export class K8sCertificatesDetailsComponent implements OnInit {
  ...
  getInstanceData(): void {
    this.isLoading = true;
    this._namespaceService.getCertificatesDetails(this.namespaceInstance).subscribe({
      next: res => {
  ...
  onDelete(item: any): void {
    ...
    dialogRef.afterClosed().subscribe((bool: boolean) => {
      if (bool === true) {
        this._namespaceService.deleteNamespaceCertificates(item?.metadata?.name).subscribe(
          res => {
