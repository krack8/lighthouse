...
@Component({
  selector: 'kc-k8s-endpoint-slice',
  templateUrl: './k8s-endpoint-slice.component.html',
  styleUrls: ['./k8s-endpoint-slice.component.scss']
})
export class K8sEndpointSliceComponent implements OnInit {
  icSearch = icSearch;
  icInfo = icInfo;
  icDelete = icDelete;
  icEdit = icEdit;
  ...
  getInstanceData(queryParam?: any): void {
    ...
    this.namespaceService.getEndpointSlice(queryParam).subscribe({
      next: res => {
        if (res?.status === 'error') {
  ...
  onDelete(item: any): void {
    ...
    dialogRef.afterClosed().subscribe((bool: boolean) => {
      if (bool === true) {
        this.namespaceService.deleteNamespaceEndpointSlice(item?.metadata?.name).subscribe(
          res => {
