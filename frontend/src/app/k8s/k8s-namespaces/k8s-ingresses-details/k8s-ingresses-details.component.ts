...
@Component({
  selector: 'kc-k8s-ingresses-details',
  templateUrl: './k8s-ingresses-details.component.html',
  styleUrls: ['./k8s-ingresses-details.component.scss']
})
export class K8sIngressesDetailsComponent implements OnInit {
  ...
  getInstanceData(): void {
    this.isLoading = true;
    this._namespaceService.getIngressDetails(this.namespaceInstance).subscribe({
      next: res => {
  ...
  onDelete(item: any): void {
    ...
    dialogRef.afterClosed().subscribe((bool: boolean) => {
      if (bool === true) {
        this._namespaceService.deleteNamespaceIngresses(item?.metadata?.name).subscribe(
          res => {
  ...
  onUpdate(item: any): void {
    ...
    dialog.componentInstance.applyManifestFor = 'ingresses';
