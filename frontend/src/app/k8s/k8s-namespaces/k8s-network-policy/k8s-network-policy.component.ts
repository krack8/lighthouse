...
@Component({
  selector: 'kc-k8s-network-policy',
  templateUrl: './k8s-network-policy.component.html',
  styleUrls: ['./k8s-network-policy.component.scss']
})
export class K8sNetworkPolicyComponent implements OnInit {
  icSearch = icSearch;
  ...
  getInstanceData(queryParam?: any): void {
    ...
    this.namespaceService.getNetwokPolicy(queryParam).subscribe({
      next: res => {
        if (res?.status === 'error') {
  ...
  onDelete(item: any): void {
    ...
    dialogRef.afterClosed().subscribe((bool: boolean) => {
      if (bool === true) {
        this.namespaceService.deleteNamespaceNetworkPolicy(item?.metadata?.name).subscribe(
          res => {
