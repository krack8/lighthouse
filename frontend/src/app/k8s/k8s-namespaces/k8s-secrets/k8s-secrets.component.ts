...
@Component({
  selector: 'kc-k8s-secrets',
  templateUrl: './k8s-secrets.component.html',
  styleUrls: ['./k8s-secrets.component.scss']
})
export class K8sSecretsComponent implements OnInit {
  ...
  getInstanceData(queryParam?: any): void {
    ...
    this.namespaceService.getSecrets(queryParam).subscribe({
      next: res => {
        if (res?.status === 'error') {
  ...
  onDelete(item: any): void {
    ...
    dialogRef.afterClosed().subscribe((bool: boolean) => {
      if (bool === true) {
        this.namespaceService.deleteNamespaceSecrets(item?.metadata?.name).subscribe(
          res => {
