...
export class K8sRoleBindingComponent implements OnInit {
  ...
  getInstanceData(queryParam?: any): void {
    ...
    this.namespaceService.getRoleBindings(queryParam).subscribe({
      next: res => {
        if (res?.status === 'error') {
  ...
  onDelete(item: any): void {
    ...
    dialogRef.afterClosed().subscribe((bool: boolean) => {
      if (bool === true) {
        this.namespaceService.deleteNamespaceRoleBinding(item?.metadata?.name).subscribe(
          res => {
