...
@Component({
  selector: 'kc-k8s-replica-sets',
  templateUrl: './k8s-replica-sets.component.html',
  styleUrls: ['./k8s-replica-sets.component.scss']
})
export class K8sReplicaSetsComponent implements OnInit {
  icCircle = icCircle;
  icSearch = icSearch;
  icInfo = icInfo;
  icDelete = icDelete;
  ...
  getInstanceData(queryParam?: any): void {
    ...
    this.namespaceService.getReplicaSets(queryParam).subscribe({
      next: res => {
        if (res?.status === 'error') {
  ...
  onDelete(item: any): void {
    ...
    dialogRef.afterClosed().subscribe((bool: boolean) => {
      if (bool === true) {
        this.namespaceService.deleteNamespaceReplicasets(item?.metadata?.name).subscribe(
          res => {
