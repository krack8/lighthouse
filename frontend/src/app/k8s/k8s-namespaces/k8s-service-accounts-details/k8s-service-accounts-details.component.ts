...
@Component({
  selector: 'kc-k8s-service-accounts-details',
  templateUrl: './k8s-service-accounts-details.component.html',
  styles: []
})
export class K8sServiceAccountsDetailsComponent implements OnInit {
  ...
  getInstanceData(): void {
    this.isLoading = true;
    this._namespaceService.getServiceAccountsDetails(this.namespaceInstance).subscribe({
      next: res => {
  ...
  onDelete(item: any): void {
    ...
    dialogRef.afterClosed().subscribe((bool: boolean) => {
      if (bool === true) {
        this._namespaceService.deleteNamespaceServiceAccount(item?.metadata?.name).subscribe(
          res => {
