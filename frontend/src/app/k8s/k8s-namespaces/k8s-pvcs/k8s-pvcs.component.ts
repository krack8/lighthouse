...
@Component({
  selector: 'kc-k8s-pvcs',
  templateUrl: './k8s-pvcs.component.html',
  styleUrls: ['./k8s-pvcs.component.scss']
})
export class K8sPvcsComponent implements OnInit {
  icSearch = icSearch;
  icInfo = icInfo;
  icDelete = icDelete;
  icAdd = icAdd;
  ...
  icCross = icCross;
  searchBy = 'name';
  title: any = 'Persistent Volume Claims';
  ...
  getInstanceData(queryParam?: any): void {
    ...
    this.namespaceService.getPvcList(queryParam).subscribe({
      next: res => {
        if (res?.status === 'error') {
  ...
  onDelete(item: any): void {
    ...
    dialogRef.afterClosed().subscribe((bool: boolean) => {
      if (bool === true) {
        this.namespaceService.deleteNamespacePvc(item?.metadata?.name).subscribe(
          res => {
