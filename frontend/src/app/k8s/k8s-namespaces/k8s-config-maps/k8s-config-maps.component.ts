...
export class K8sConfigMapsComponent implements OnInit {
  ...
  getInstanceData(queryParam?: any): void {
    ...
    this.namespaceService.getConfigMaps(queryParam).subscribe({
      next: res => {
        if (res?.status === 'error') {
  ...
  onDelete(item: any): void {
    ...
    dialogRef.afterClosed().subscribe((bool: boolean) => {
      if (bool === true) {
        this.namespaceService.deleteNamespaceConfigMap(item?.metadata?.name).subscribe(
          res => {
