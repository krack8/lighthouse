...
@Component({
  selector: 'kc-k8s-role',
  templateUrl: './k8s-role.component.html',
  styleUrls: ['./k8s-role.component.scss']
})
export class K8sRoleComponent implements OnInit {
  icSearch = icSearch;
  ...
  getInstanceData(queryParam?: any): void {
    ...
    this.namespaceService.getRole(queryParam).subscribe({
      next: res => {
        if (res?.status === 'error') {
  ...
  onDelete(item: any): void {
    ...
    dialogRef.afterClosed().subscribe((bool: boolean) => {
      if (bool === true) {
        this.namespaceService.deleteNamespaceRole(item?.metadata?.name).subscribe(
          res => {
