...
@Component({
  selector: 'kc-k8s-endpoints-details',
  templateUrl: './k8s-endpoints-details.component.html',
  styleUrls: ['./k8s-endpoints-details.component.scss']
})
export class K8sEndpointsDetailsComponent implements OnInit {
  ...
  getInstanceData(): void {
    this.isLoading = true;
    this._namespaceService.getEndpointsDetails(this.namespaceInstance).subscribe({
      next: res => {
  ...
  onDelete(item: any): void {
    ...
    dialogRef.afterClosed().subscribe((bool: boolean) => {
      if (bool === true) {
        this._namespaceService.deleteNamespaceEndpoint(item?.metadata?.name).subscribe(
          res => {
