...
@Component({
  selector: 'kc-k8s-virtual-service',
  templateUrl: './k8s-virtual-service.component.html',
  styleUrls: ['./k8s-virtual-service.component.scss']
})
export class K8sVirtualServiceComponent implements OnInit {
  icSearch = icSearch;
  ...
  getInstanceData(queryParam?: any): void {
    ...
    this.namespaceService.getVirtualService(queryParam).subscribe({
      next: res => {
        if (res?.status === 'error') {
  ...
  onDelete(item: any): void {
    ...
    dialogRef.afterClosed().subscribe((bool: boolean) => {
      if (bool === true) {
        this.namespaceService.deleteNamespaceVirtualService(item?.metadata?.name).subscribe(
          res => {
