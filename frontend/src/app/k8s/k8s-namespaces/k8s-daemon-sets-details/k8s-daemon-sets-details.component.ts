...
@Component({
  selector: 'kc-k8s-daemon-sets-details',
  templateUrl: './k8s-daemon-sets-details.component.html',
  styleUrls: ['./k8s-daemon-sets-details.component.scss']
})
export class K8sDaemonSetsDetailsComponent implements OnInit {
  icCircle = icCircle;
  ...
  getInstanceData(): void {
    this.isLoading = true;
    this._namespaceService.getDaemonSetsDetails(this.namespaceInstance).subscribe({
      next: res => {
      ...
      error: err => {
        this.isLoading = false;
        this.toastr.error('Failed: ', err.error.message || 'Something went wrong');
      }
    });
  }
  onDelete(item: any): void {
    ...
    dialogRef.afterClosed().subscribe((bool: boolean) => {
      if (bool === true) {
        this._namespaceService.deleteNamespaceDaemonset(item?.metadata?.name).subscribe(
          res => {
