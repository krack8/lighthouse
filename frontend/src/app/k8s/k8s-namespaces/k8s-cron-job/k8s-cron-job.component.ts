...
@Component({
  selector: 'kc-k8s-cron-job',
  templateUrl: './k8s-cron-job.component.html',
  styleUrls: ['./k8s-cron-job.component.scss']
})
export class K8sCronJobComponent implements OnInit {
  icSearch = icSearch;
  ...
  getInstanceData(queryParam?: any): void {
    ...
    this.namespaceService.getCronJob(queryParam).subscribe({
      next: res => {
        if (res?.status === 'error') {
  ...
  onDelete(item: any): void {
    ...
    dialogRef.afterClosed().subscribe((bool: boolean) => {
      if (bool === true) {
        this.namespaceService.deleteCronJob(item?.metadata?.name).subscribe(
          res => {
