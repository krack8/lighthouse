...
import { distinctUntilChanged, map, share, takeUntil, takeWhile, throttleTime } from 'rxjs/operators';
import { K8sNamespacesService } from '../k8s-namespaces.service';

@Component({
  selector: 'kc-k8s-deployments',
  templateUrl: './k8s-deployments.component.html',
  styleUrls: ['./k8s-deployments.component.scss'],
  animations: [fadeInRight400ms]
})
export class K8sDeploymentsComponent implements OnInit, OnDestroy {
  private _destroy$: ReplaySubject<boolean> = new ReplaySubject<boolean>(0);
  icCircle = icCircle;
  icSearch = icSearch;
  ...
  getInstanceData(queryParam?: any): void {
    this.getDeploymentStats(queryParam);
  ...
  getDeploymentStats(queryParam?: any): void {
    this.statsLoaded = true;
    this.namespaceService.getDeploymentStats(queryParam).subscribe({
      next: res => {
        if (res?.status === 'error') {
          this.toastr.error(res?.message);
        }
  ...
  onDelete(item: any): void {
    ...
    dialogRef.afterClosed().subscribe((bool: boolean) => {
      if (bool === true) {
        this.namespaceService.deleteNamespaceDeployment(item?.metadata?.name).subscribe(
          res => {
