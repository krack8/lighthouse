...
@Component({
  selector: 'kc-k8s-service-accounts',
  templateUrl: './k8s-service-accounts.component.html',
  styleUrls: ['./k8s-service-accounts.component.scss']
})
export class K8sServiceAccountsComponent implements OnInit {
  icSearch = icSearch;
  ...
  getInstanceData(queryParam?: any): void {
    ...
    this.namespaceService.getServiceAccounts(queryParam).subscribe({
      next: res => {
        if (res?.status === 'error') {
  ...
  onDelete(item: any): void {
    ...
    dialogRef.afterClosed().subscribe((bool: boolean) => {
      if (bool === true) {
        this.namespaceService.deleteNamespaceServiceAccount(item?.metadata?.name).subscribe(
          res => {
