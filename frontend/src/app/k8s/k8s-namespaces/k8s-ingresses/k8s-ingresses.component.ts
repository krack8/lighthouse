...
export class K8sIngressesComponent implements OnInit {
  ...
  getInstanceData(queryParam?: any): void {
    ...
    this.namespaceService.getIngresses(queryParam).subscribe({
      next: res => {
        if (res?.status === 'error') {
  ...
  onDelete(item: any): void {
    ...
    dialogRef.afterClosed().subscribe((bool: boolean) => {
      if (bool === true) {
        this.namespaceService.deleteNamespaceIngresses(item?.metadata?.name).subscribe(
          res => {
