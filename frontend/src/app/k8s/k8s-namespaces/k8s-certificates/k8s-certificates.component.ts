...
@Component({
  selector: 'kc-k8s-certificates',
  templateUrl: './k8s-certificates.component.html',
  styleUrls: ['./k8s-certificates.component.scss']
})
export class K8sCertificatesComponent implements OnInit {
  icSearch = icSearch;
  icInfo = icInfo;
  icDelete = icDelete;
  icEdit = icEdit;
  ...
  getInstanceData(queryParam?: any): void {
    ...
    this.namespaceService.getCertificates(queryParam).subscribe({
      next: res => {
        if (res?.status === 'error') {
  ...
  onDelete(item: any): void {
    ...
    dialogRef.afterClosed().subscribe((bool: boolean) => {
      if (bool === true) {
        this.namespaceService.deleteNamespaceCertificates(item?.metadata?.name).subscribe(
          res => {
