...
@Component({
  selector: 'kc-k8s-secrets-details',
  templateUrl: './k8s-secrets-details.component.html',
  styleUrls: ['./k8s-secrets-details.component.scss']
})
export class K8sSecretsDetailsComponent implements OnInit {
  ...
  getInstanceData(): void {
    this.isLoading = true;
    this._namespaceService.getSecretsDetails(this.namespaceInstance).subscribe({
      next: res => {
  ...
  onDelete(item: any): void {
    ...
    dialogRef.afterClosed().subscribe((bool: boolean) => {
      if (bool === true) {
        this._namespaceService.deleteNamespaceSecrets(item?.metadata?.name).subscribe(
          res => {
