...
@Component({
  selector: 'kc-k8s-resource-quota-details',
...
export class K8sResourceQuotaDetailsComponent implements OnInit {
  ...
  getInstanceData(): void {
    this.isLoading = true;
    this._namespaceService.getResourceQuotaDetails(this.namespaceInstance).subscribe({
      next: res => {
  ...
  onDelete(item: any): void {
    ...
    dialogRef.afterClosed().subscribe((bool: boolean) => {
      if (bool === true) {
        this._namespaceService.deleteNamespaceResourceQuota(item?.metadata?.name).subscribe(
          res => {
