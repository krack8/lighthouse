...
@Component({
  selector: 'kc-k8s-gateway-details',
  templateUrl: './k8s-gateway-details.component.html',
  styleUrls: ['./k8s-gateway-details.component.scss']
})
export class K8sGatewayDetailsComponent implements OnInit {
  ...
  getInstanceData(): void {
    this.isLoading = true;
    this._namespaceService.getGatewayDetails(this.namespaceInstance).subscribe({
      next: res => {
  ...
  onDelete(item: any): void {
    ...
    dialogRef.afterClosed().subscribe((bool: boolean) => {
      if (bool === true) {
        this._namespaceService.deleteNamespaceGateway(item?.metadata?.name).subscribe(
          res => {
