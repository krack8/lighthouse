...
@Component({
  selector: 'kc-k8s-virtual-service-details',
  templateUrl: './k8s-virtual-service-details.component.html',
  styleUrls: ['./k8s-virtual-service-details.component.scss']
})
export class K8sVirtualServiceDetailsComponent implements OnInit {
  ...
  getInstanceData(): void {
    this.isLoading = true;
    this._namespaceService.getVirtualServiceDetails(this.namespaceInstance).subscribe({
      next: res => {
  ...
  onDelete(item: any): void {
    ...
    dialogRef.afterClosed().subscribe((bool: boolean) => {
      if (bool === true) {
        this._namespaceService.deleteNamespaceVirtualService(item?.metadata?.name).subscribe(
          res => {
