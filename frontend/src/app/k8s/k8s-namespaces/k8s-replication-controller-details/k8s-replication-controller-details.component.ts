...
@Component({
  selector: 'kc-k8s-replication-controller-details',
  templateUrl: './k8s-replication-controller-details.component.html',
  styleUrls: ['./k8s-replication-controller-details.component.scss']
})
export class K8sReplicationControllerDetailsComponent implements OnInit {
  ...
  getInstanceData(): void {
    this.isLoading = true;
    this._namespaceService.getReplicationControllerDetails(this.namespaceInstance).subscribe({
      next: res => {
  ...
  onDelete(item: any): void {
    ...
    dialogRef.afterClosed().subscribe((bool: boolean) => {
      if (bool === true) {
        this._namespaceService.deleteNamespaceReplicationController(item?.metadata?.name).subscribe(
          res => {
