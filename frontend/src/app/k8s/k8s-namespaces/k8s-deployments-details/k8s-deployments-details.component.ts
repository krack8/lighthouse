...
@Component({
  selector: 'kc-k8s-deployments-details',
  templateUrl: './k8s-deployments-details.component.html',
  styleUrls: ['./k8s-deployments-details.component.scss'],
  animations: [fadeInRight400ms]
})
export class K8sDeploymentsDetailsComponent implements OnInit {
  icCircle = icCircle;
  ...
  getInstanceData(): void {
    this.isLoading = true;
    this._namespaceService.getDeploymentsDetailsV1(this.namespaceInstance).subscribe({
      next: res => {
        if (res?.data) {
          this.data = res?.data;
          this.metadataData = this.data;
          this.getPodList();
  ...
  onDelete(item: any): void {
    ...
    dialogRef.afterClosed().subscribe((bool: boolean) => {
      if (bool === true) {
        this._namespaceService.deleteNamespaceDeployment(item?.metadata?.name).subscribe(
          res => {
  ...
  getPodList(): void {
    ...
    for (let item of this.data?.status?.conditions) {
    ...
    if (replicaset) {
      this.podListData = {
        deploymentName: this.data.metadata.name,
  ...
  isConditionNegative(condition: any): boolean {
