...
@Component({
  selector: 'kc-k8s-replication-controller',
  templateUrl: './k8s-replication-controller.component.html',
  styleUrls: ['./k8s-replication-controller.component.scss']
})
export class K8sReplicationControllerComponent implements OnInit {
  icSearch = icSearch;
  icInfo = icInfo;
  icDelete = icDelete;
  icEdit = icEdit;
  ...
  icRefresh = icRefresh;
  searchBy = 'name';
  title: any = 'Replication Controller';
  ...
  getInstanceData(queryParam?: any): void {
    ...
    this.namespaceService.getReplicationController(queryParam).subscribe({
      next: res => {
        if (res?.status === 'error') {
  ...
  onDelete(item: any): void {
    ...
    dialogRef.afterClosed().subscribe((bool: boolean) => {
      if (bool === true) {
        this.namespaceService.deleteNamespaceReplicationController(item?.metadata?.name).subscribe(
          res => {
  ...
  loadMore() {
    ...
    this.namespaceService
      .getReplicationController(queryParam)
